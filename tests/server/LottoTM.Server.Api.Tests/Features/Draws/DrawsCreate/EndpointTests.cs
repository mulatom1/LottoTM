...
namespace LottoTM.Server.Api.Tests.Features.Draws.DrawsCreate;

/// <summary>
/// Integration tests for the CreateDraw endpoint
/// </summary>
public class EndpointTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    /// <summary>
    /// Test successful draw creation with valid data
    /// </summary>
    [Fact]
    public async Task CreateDraw_WithValidData_Returns201Created()
    {
        // Arrange
        var testDbName = "TestDb_ValidDraw_" + Guid.NewGuid();
        ...
        // Act
        var response = await client.PostAsJsonAsync("/api/draws", request);

        // Assert
        Assert.Equal(HttpStatusCode.Created, response.StatusCode);
        var result = await response.Content.ReadFromJsonAsync<Dictionary<string, string>>();
        Assert.NotNull(result);
        Assert.Equal("Losowanie utworzone pomyślnie", result["message"]);

        // Verify in database
        VerifyDrawInDatabase(factory, DateOnly.Parse("2025-01-15"), new[] { 3, 12, 25, 31, 42, 48 });
    }

    /// <summary>
    /// Test draw creation with future date returns 400
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Test draw creation with invalid number count returns 400
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Test draw creation with numbers out of range returns 400
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Test draw creation with duplicate numbers returns 400
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Test draw creation with duplicate date returns 400
    /// </summary>
    [Fact]
    public async Task CreateDraw_WithDuplicateDate_Returns400BadRequest()
    {
        // Arrange
        ...
        // Seed existing draw
        SeedTestDraw(factory, userId, DateOnly.Parse("2025-01-15"), new[] { 1, 2, 3, 4, 5, 6 });
    ...
    /// <summary>
    /// Test draw creation without authentication returns 401
    /// </summary>
    [Fact]
    public async Task CreateDraw_WithoutAuthentication_Returns401Unauthorized()
    {
        // Arrange
    ...
    /// <summary>
    /// Helper method to verify draw was created in database
    /// </summary>
    private static void VerifyDrawInDatabase(WebApplicationFactory<Program> factory, DateOnly drawDate, int[] expectedNumbers)
    {
        using var scope = factory.Services.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<AppDbContext>();
        ...
        var draw = dbContext.Draws
            .Include(d => d.Numbers)
            .FirstOrDefault(d => d.DrawDate == drawDate);
